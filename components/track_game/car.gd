...
var forward_velocity: float = 300
...
func steer(track_segments: Array[TrackSegment]):
	...
	# set the car to be facing perpendicular to the line between the two points
	var facing_vector = (outer_point_global - inner_point_global).rotated(deg_to_rad(90))
	self.global_rotation = facing_vector.angle()
